...
#define OBSERVATIONS_COUNT                                                     \
    (PLAYER_OBSERVATIONS_COUNT + GHOST_OBSERVATIONS_COUNT * NUM_GHOSTS +       \
     NUM_DOTS)

#define PINKY_CORNER                                                           \
    (IVector2) { 3, -3 }
#define BLINKY_CORNER                                                          \
    (IVector2) { MAP_WIDTH - 4, -3 }
#define INKY_CORNER                                                            \
    (IVector2) { MAP_WIDTH - 1, MAP_HEIGHT }
#define CLYDE_CORNER                                                           \
    (IVector2) { 0, MAP_HEIGHT }
...
void init(PacmanEnv *env) {
    ...
    env->pickup_obs = (float *)calloc(NUM_DOTS, sizeof(float));
    env->pickup_obs_map =
        (float **)calloc(MAP_WIDTH * MAP_HEIGHT, sizeof(float *));
    ...
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) {
            ...
            if (tile == DOT_TILE || tile == POWER_TILE) {
                env->pickup_obs_map[y * MAP_WIDTH + x] =
                    &env->pickup_obs[dot_count];
                dot_count++;
            } else {
                env->pickup_obs_map[y * MAP_WIDTH + x] = NULL;
            }
        }
    }
}
...
static inline bool vec2_equal(IVector2 a, IVector2 b) {
    return a.x == b.x && a.y == b.y;
}

static inline int vec2_distance_squared(IVector2 pos, IVector2 target) {
    int dx = pos.x - target.x;
    int dy = pos.y - target.y;
    return dx * dx + dy * dy;
}

static inline IVector2 vec2_move(IVector2 pos, Direction direction, int distance) {
    switch (direction) {
    case UP:
        pos.y -= distance;
        break;
...
static inline IVector2 vec2_move_wrapped(IVector2 pos, Direction direction, int distance) {
...
static inline Tile *tile_at(PacmanEnv *env, IVector2 pos) {
    return &env->game_map[pos.y * MAP_WIDTH + pos.x];
}

static inline bool can_move_in_direction(PacmanEnv *env, IVector2 pos, Direction direction) {
    return *tile_at(env, vec2_move_wrapped(pos, direction, 1)) != WALL_TILE;
}

void compute_ghost_observations(PacmanEnv *env, Ghost *ghost,
                                float *observations, int i) {
    observations[i + 0] = ghost->pos.x / (float) MAP_WIDTH;
    observations[i + 1] = ghost->pos.y / (float) MAP_HEIGHT;
    observations[i + 2] = ghost->direction == UP;
    observations[i + 3] = ghost->direction == DOWN;
    observations[i + 4] = ghost->direction == LEFT;
    observations[i + 5] = ghost->direction == RIGHT;
    observations[i + 6] = !ghost->frightened && !ghost->return_to_spawn;
    observations[i + 7] = ghost->frightened;
    observations[i + 8] = ghost->return_to_spawn;
}
...
void compute_observations(PacmanEnv *env) {
    ...
    env->observations[10] =
        env->frightened_time_left / (float)env->frightened_time;

    // ghost observations
    compute_ghost_observations(env, &env->pinky, env->observations,
                               PLAYER_OBSERVATIONS_COUNT);
    compute_ghost_observations(env, &env->blinky, env->observations,
                               PLAYER_OBSERVATIONS_COUNT +
                                   GHOST_OBSERVATIONS_COUNT);
    compute_ghost_observations(env, &env->inky, env->observations,
                               PLAYER_OBSERVATIONS_COUNT +
                                   (GHOST_OBSERVATIONS_COUNT * 2));
    compute_ghost_observations(env, &env->clyde, env->observations,
                               PLAYER_OBSERVATIONS_COUNT +
                                   (GHOST_OBSERVATIONS_COUNT * 3));

    memcpy(env->observations + PLAYER_OBSERVATIONS_COUNT +
               (GHOST_OBSERVATIONS_COUNT * 4),
           env->pickup_obs, NUM_DOTS * sizeof(float));
    // dot observations
    // for (int i = 0; i < NUM_DOTS; i++) {
    //   env->observations[i + PLAYER_OBSERVATIONS_COUNT +
    //   (GHOST_OBSERVATIONS_COUNT * 4)] = *env->dots[i] != EMPTY_TILE;
    // }
}
...
void reset_ghost(PacmanEnv *env, Ghost *ghost) {
    ghost->direction = UP;
    ghost->start_timeout =
        rand_range(env->min_start_timeout, env->max_start_timeout);
...
void reset_round(PacmanEnv *env) {
    Tile original_tile;
    Tile *map_tile;
    int player_randomizer = rand() % 240;
    ...
    env->step_count = 0;
    env->remaining_pickups = NUM_DOTS;
        ...
        for (int x = 0; x < MAP_WIDTH; x++) {
            ...
            case DOT_TILE:
                if (env->randomize_starting_position &&
                    ...
                    env->player_pos = (IVector2){x, y};
                    *map_tile = EMPTY_TILE;
                    if (env->pickup_obs_map[y * MAP_WIDTH + x] != NULL)
                        *env->pickup_obs_map[y * MAP_WIDTH + x] = 0.0f;
...
void player_move(PacmanEnv *env, Direction action) {
    ...
    if (*new_tile != WALL_TILE) {
        ...
        if (*new_tile == DOT_TILE) {
            ...
            env->remaining_pickups--;
            *new_tile = EMPTY_TILE;
            if (env->pickup_obs_map[new_pos.y * MAP_WIDTH + new_pos.x] != NULL)
                *env->pickup_obs_map[new_pos.y * MAP_WIDTH + new_pos.x] = 0.0f;
...
static inline int ghost_movement_options(PacmanEnv *env, Ghost *ghost,
                           Direction *directions) {
    int option_count = 0;

    if (ghost->direction != DOWN &&
        can_move_in_direction(env, ghost->pos, UP)) {
        directions[option_count++] = UP;
    }
    if (ghost->direction != RIGHT &&
        can_move_in_direction(env, ghost->pos, LEFT)) {
        directions[option_count++] = LEFT;
    }
    if (ghost->direction != UP &&
        can_move_in_direction(env, ghost->pos, DOWN)) {
        directions[option_count++] = DOWN;
    }
    if (ghost->direction != LEFT &&
        can_move_in_direction(env, ghost->pos, RIGHT)) {
...
Direction ghost_random_walk(PacmanEnv *env, Ghost *ghost) {
    Direction directions[4];
    int option_count = ghost_movement_options(env, ghost, directions);
    if (option_count == 0)
        return ghost->direction; // nowhere to go
...
Direction ghost_move_to_target(PacmanEnv *env, Ghost *ghost) {
    Direction directions[4];
    int distances[4];
    int option_count = ghost_movement_options(env, ghost, directions);
    ...
    for (int i = 0; i < option_count; i++) {
        distances[i] = vec2_distance_squared(
            vec2_move(ghost->pos, directions[i], 1), ghost->target);
    }
...
void set_chase_targets(PacmanEnv *env) {
    ...
    env->pinky.target =
        vec2_move(env->player_pos, env->player_direction, PINKY_TARGET_LEAD);

    env->inky.target =
        vec2_move(env->player_pos, env->player_direction, INKY_TARGET_LEAD);
...
static inline bool check_collision(IVector2 a, IVector2 old_a, IVector2 b, IVector2 old_b) {
...
void ghost_move(PacmanEnv *env, Ghost *ghost, IVector2 old_player_pos) {
    ...
    if (env->reverse_directions && !ghost->return_to_spawn) {
        ...
        ghost->direction = ghost_random_walk(env, ghost);
    } else {
        ghost->direction = ghost_move_to_target(env, ghost);
    }
    ...
    } else if (check_collision(ghost->pos, old_ghost_position, env->player_pos,
                               old_player_pos)) {
        if (ghost->frightened) {
            ghost->frightened = false;
...
void c_step(PacmanEnv *env) {
    ...
    if (env->player_caught || env->step_count >= MAX_STEPS ||
        env->remaining_pickups <= 0) {
        // env->log.score = env->score;
        // env->log.episode_return = env->score;
        // env->log.episode_length = env->step_count;
        add_log(env);
...
Vector2 lerp_ivector2(IVector2 a, IVector2 b, float progress) {
    ...
    return (Vector2){a_x + (b_x - a_x) * progress,
                     a_y + (b_y - a_y) * progress};
}

void draw_tiled(Client *client, Texture2D texture, Vector2 position,
                float rotation, bool flip_x, float source_width,
                float source_height) {
    Rectangle source =
        (Rectangle){0, 0, flip_x ? -source_width : source_width, source_height};
...
void draw_entity(Client *client, Texture2D texture, IVector2 previous_pos,
                 IVector2 pos, float progress, float rotation, bool flip_x,
                 float source_width, float source_height) {
    Vector2 position;

    if (pos.x == 0 && previous_pos.x == MAP_WIDTH - 1) {
        position = lerp_ivector2((IVector2){-1, previous_pos.y}, pos, progress);
        draw_tiled(client, texture, position, rotation, flip_x, source_width,
                   source_height);
        ...
        position =
            lerp_ivector2((IVector2){MAP_WIDTH, previous_pos.y}, pos, progress);
        draw_tiled(client, texture, position, rotation, flip_x, source_width,
                   source_height);
...
Client *make_client(PacmanEnv *env) {
    ...
    InitWindow(client->tile_size * MAP_WIDTH,
               client->tile_size * MAP_HEIGHT + PX_PADDING_TOP,
               "PufferLib Pacman");
    SetTargetFPS(60);
...
void render_ghost(Client *client, Ghost *ghost, DirectionSprites *sprites,
                  IVector2 previous_pos, float progress) {
    Texture2D texture;
    ...
    draw_entity(client, texture, previous_pos, ghost->pos, progress, 0.0f,
                false, 16, 16);
}
...
void render_player(Client *client, PacmanEnv *env, float progress) {
    ...
    draw_entity(client, client->pacman, client->previous_player_pos,
                env->player_pos, progress, rotation,
                env->player_direction == LEFT, 128, 128);
}
...
int get_tile_index(PacmanEnv *env, IVector2 pos) {
    ...
    if (is_wall(env, (IVector2){pos.x, pos.y - 1}))
        tile_bits |= TILE_N;
    if (is_wall(env, (IVector2){pos.x + 1, pos.y - 1}))
        tile_bits |= TILE_NE;
    if (is_wall(env, (IVector2){pos.x + 1, pos.y}))
        tile_bits |= TILE_E;
    if (is_wall(env, (IVector2){pos.x + 1, pos.y + 1}))
        tile_bits |= TILE_SE;
    if (is_wall(env, (IVector2){pos.x, pos.y + 1}))
    ...
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) {
            ...
            if (tile == WALL_TILE) {
                ...
                DrawRectangle(x * client->tile_size + client->tile_size / 2.0f -
                                  width / 2.0f,
                              y * client->tile_size + client->tile_size / 2.0f -
                                  height / 2.0f + PX_PADDING_TOP,
...
void c_render(PacmanEnv *env) {
    ...
    render_ghost(client, &env->pinky, &client->pinky,
                 client->previous_pinky_pos, progress);
    render_ghost(client, &env->blinky, &client->blinky,
                 client->previous_blinky_pos, progress);
    render_ghost(client, &env->inky, &client->inky, client->previous_inky_pos,
                 progress);
    render_ghost(client, &env->clyde, &client->clyde,
                 client->previous_clyde_pos, progress);
